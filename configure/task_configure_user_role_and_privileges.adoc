...
+
Consulta el artículo de KB - https://kb.netapp.com/mgmt/OTV/Virtual_Storage_Console/Virtual_Storage_Console%3A_How_to_retrieve_the_JSON_file_to_configure_user_roles_and_privileges["Virtual Storage Console: Cómo recuperar el archivo JSON para configurar los roles y privilegios de usuario"^] para ver instrucciones sobre cómo descargar el archivo ONTAP Privileges desde Web-CLI.
