...
Si el sistema de almacenamiento se añade con un usuario en el ámbito del clúster creado personalizado mediante el archivo json y desea actualizar a la versión 9,12, ejecute los siguientes comandos en la interfaz de línea de comandos de ONTAP antes de la actualización para habilitar la comunicación basada en certificados entre las herramientas de ONTAP para VMware vSphere y ONTAP:
